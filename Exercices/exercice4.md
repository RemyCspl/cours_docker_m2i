...
```bash
docker run -d --name jeu-remy -p 8080:8080 quchaonet/2048
```
...
```bash
docker run -d --name jeu2-remy -p 8081:8080 quchaonet/2048
```
...
```bash
docker stop jeu-remy
docker stop jeu2-remy
...
```
Lié à l'option -p 8080:80 de la commande docker run
```
...
```
On va sur la page http://localhost:8080 et la page internet affiche bien la page d'accueil par défaut de nginx
```
...
```
http://localhost:8081 affiche bien le nouveau texte
```
...
- « nginx-web2 ».
```bash
docker run -d -p 8079:80 --name nginx-web2 nginx
```

- « nginx-web3 ».
```bash
docker run -d -p 8078:80 --name nginx-web3 nginx
```
...
```bash
docker cp "C:\Users\Administrateur\Documents\.repos\cours_docker_m2i\Exercices_J1\files_tp_conteneur\html"  nginx-web2:/usr/share/nginx/
```

- html5up-massively.zip pour nginx-web3
```bash
docker cp "C:\Users\Administrateur\Documents\.repos\cours_docker_m2i\Exercices_J1\files_tp_conteneur\html"  nginx-web3:/usr/share/nginx/
```

- html5up-paradigm-shift.zip pour nginx-web4
```bash
docker cp "C:\Users\Administrateur\Documents\.repos\cours_docker_m2i\Exercices_J1\files_tp_conteneur\html"  nginx-web4:/usr/share/nginx/
```
...
# Supprimer tous les containers avec le statut stopped
